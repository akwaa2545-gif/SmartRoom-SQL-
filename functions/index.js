...
function assertSearchQuery(value) {
  ...
  if (query.length < 1 || query.length > 254) {
    throw new HttpsError("invalid-argument", "query must be between 1 and 254 characters.");
  }
...
// Browser transactions cannot read a room-schedule query. This callable keeps
// the availability query and booking write together in one Admin transaction.
const CONCURRENCY_BOOKING_FIELDS = new Set([
  "id", "roomId", "title", "organizer", "department", "employeeId", "deskNumber", "email",
  "emailDisplayName", "emailJobTitle", "emailDepartment", "createdByUid", "startTime", "endTime",
  "status", "createdAt", "actualStartTime", "actualEndTime", "verifiedAt", "verificationEmailStatus",
  "verificationEmailScheduledAt", "verificationWindowOpenedAt", "verificationWindowClosedAt",
  "verificationEmailSentAt", "verificationEmailFailedAt",
]);
const CONCURRENCY_TIME_FIELDS = new Set([
  "startTime", "endTime", "createdAt", "actualStartTime", "actualEndTime", "verifiedAt",
  "verificationEmailScheduledAt", "verificationWindowOpenedAt", "verificationWindowClosedAt",
  "verificationEmailSentAt", "verificationEmailFailedAt",
]);

function concurrencyTimestamp(value, field) {
  if (value instanceof admin.firestore.Timestamp) return value;
  const date = new Date(String(value));
  if (Number.isNaN(date.getTime())) throw new HttpsError("invalid-argument", `${field} must be a valid date.`);
  return admin.firestore.Timestamp.fromDate(date);
}

function concurrencyBookingData(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new HttpsError("invalid-argument", "Booking details are invalid.");
  }
  const output = {};
  for (const [field, value] of Object.entries(input)) {
    if (!CONCURRENCY_BOOKING_FIELDS.has(field) || value === undefined) continue;
    output[field] = CONCURRENCY_TIME_FIELDS.has(field) && value !== null ? concurrencyTimestamp(value, field) : value;
  }
  return output;
}

function concurrencyBlocksRoom(status) {
  return status !== "REJECTED" && status !== "NO_SHOW";
}

exports.saveBookingWithConcurrency = onCall(APP_HTTPS_OPTIONS, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in is required before booking a room.");
  const operation = request.data?.operation;
  const bookingId = typeof request.data?.bookingId === "string" ? request.data.bookingId.trim() : "";
  const changes = concurrencyBookingData(request.data?.booking);
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (operation === "create" && currentSnapshot.exists) throw new HttpsError("already-exists", "This booking was already created.");
    if (operation === "update" && !currentSnapshot.exists) throw new HttpsError("not-found", "Booking was not found.");

    const candidate = { ...(currentSnapshot.exists ? currentSnapshot.data() : {}), ...changes, id: bookingId };
    ...
    const startTime = concurrencyTimestamp(candidate.startTime, "startTime");
    const endTime = concurrencyTimestamp(candidate.endTime, "endTime");
    ...
    const conflict = concurrencyBlocksRoom(candidate.status) && roomBookings.docs.some((snapshot) => {
      if (snapshot.id === bookingId) return false;
      const existing = snapshot.data();
      return concurrencyBlocksRoom(existing.status) && existing.startTime && existing.endTime &&
        existing.startTime.toMillis() < endTime.toMillis() && existing.endTime.toMillis() > startTime.toMillis();
    });
