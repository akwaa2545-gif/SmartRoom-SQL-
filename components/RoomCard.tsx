...
import {
  BookingDisplayState,
  getBookingDisplayState as getSharedBookingDisplayState,
} from "../utils/bookingStatus";
import {
  getBookingDepartmentBadgeClass,
  getBookingDepartmentClassForState,
  getBookingDepartmentDotClass,
} from "../bookingVisualStyles";
...
const RoomCard: React.FC<RoomCardProps> = ({
  ...
  const now = new Date();
  const temporarilyDisabledLabel =
    language === "th" ? "ปิดใช้งานชั่วคราว" : "Temporarily Disabled";
  const checkInWindowTooltip =
    language === "th"
  ...
  const getBookingDisplayState = (booking: Booking): BookingDisplayState =>
    getSharedBookingDisplayState(booking, now);
  ...
  const getBookingStatusBadgeClass = (
    ...
    const departmentBadgeClass = department
      ? getBookingDepartmentBadgeClass(department)
      : "";
    if (state === "roomInUse")
      return "bg-rose-500 text-white border border-rose-300 ring-2 ring-rose-100 shadow-md animate-pulse";
    if (departmentBadgeClass) return departmentBadgeClass;
    if (state === "waitForVerify" || state === "confirmed")
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex flex-col h-full">
      <div className="relative h-32 overflow-hidden p-4 flex flex-col justify-end z-0">
        ...
        <h3 className="text-white text-lg font-bold truncate drop-shadow-sm">
          {room.name}
        </h3>
        <p className="text-slate-100/90 text-xs font-semibold drop-shadow-sm">
      ...
      <div className="p-4 flex-grow flex flex-col">
          ...
          {(room.amenities || []).slice(0, 2).map((amenity, idx) => (
            <div key={idx} className="flex items-center">
              ...
              <span className="truncate max-w-[100px]">
                {translateText(amenity, language)}
              </span>
            </div>
          ))}
        </div>
        ...
        {isRoomCurrentlyClosed(room) && (
          <div className="mb-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600">
            ...
            <div>
              {formatHour(room.closureStartTime)} -{" "}
              {formatHour(room.closureEndTime ?? BOOKING_END_HOUR)}
            </div>
            ...
            <div className="space-y-2">
              {todaysBookings.slice(0, 2).map((booking) => {
                ...
                return (
                  <div
                    key={booking.id}
                    className={`flex justify-between items-center gap-2 text-sm rounded-lg px-2 py-1 border ${getBookingDepartmentClassForState(displayState, booking.department)}`}
                  >
                    <div className="flex items-center truncate min-w-0">
                      <span
                        className={`w-2 h-2 rounded-full mr-2 flex-shrink-0 ${getBookingDepartmentDotClass(booking.department)}`}
                      ></span>
                      ...
                      <span
                        title={
                          displayState === "waitForVerify" ||
                          displayState === "roomInUse" ||
                          displayState === "noCheckIn"
                            ? checkInWindowTooltip
                            : undefined
                        }
              ...
              {todaysBookings.length > 2 && (
                <p className="text-xs text-slate-400">
                  +{todaysBookings.length - 2} {t.more}
                </p>
              )}
            </div>
          )}
