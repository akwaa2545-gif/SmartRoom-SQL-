...
6. Run `start-api.cmd` for a manual test. It deliberately listens only on `127.0.0.1:8787`; do not expose the Node port or SQL Server directly. The host setup script creates a Windows startup task that relaunches the API after a reboot or unexpected stop.
