...
7. Run `sql/005_sql_bookings.sql` and `sql/006_sql_operational_data.sql` once. These create the SQL tables used by room booking, operations, and the leaderboard endpoints. For an existing installation, also run `sql/011_booking_actual_end_time.sql` once; it safely adds the check-out history column if missing.
8. Run `sql/009_mascot_email_assignments.sql` once to enable Admin mascot assignments by individual YAGEO email address. The older `008_mascot_assignments.sql` department table is no longer used by the API.
