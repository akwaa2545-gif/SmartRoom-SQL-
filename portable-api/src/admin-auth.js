...
function createPasswordRecord(password) {
  ...
  const hash = crypto.pbkdf2Sync(password, salt, PBKDF2_ITERATIONS, KEY_LENGTH, DIGEST).toString('base64url');
...
function verifyPassword(password, record) {
  ...
  if (scheme !== 'pbkdf2' || !Number.isSafeInteger(iterations) || iterations < 100_000 || !salt || !expectedHash) return false;
...
function createSession({ username, role, sessionVersion }, secret, expiresAt) {
  const payload = base64Json({ username, role, sessionVersion, exp: expiresAt });
...
function verifySession(token, secret, now = Date.now()) {
  ...
  try {
    ...
    if (!data || typeof data.username !== 'string' || typeof data.sessionVersion !== 'string' || !['SUPER_ADMIN', 'APPROVER'].includes(data.role) || !Number.isFinite(data.exp) || data.exp <= now) return null;
    return data;
  } catch {
    return null;
  }
...
module.exports = { createPasswordRecord, verifyPassword, createSession, verifySession };
