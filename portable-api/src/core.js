...
function assertYageoEmail(value, domain = DEFAULT_YAGEO_DOMAIN) {
  ...
  if (!new RegExp(`^[^\\s@]+@${escapedDomain}$`, 'i').test(email) || email.length > 254) {
    throw new ApiError(400, 'invalid-email', `email must be a valid @${domain} address.`);
  }
  return email;
}
...
function normalizeUser(user) {
  ...
  return {
    ...
    userPrincipalName: string(user.userPrincipalName || user.userprincipalname || user.upn).toLowerCase(),
...
module.exports = {
  ApiError, assertYageoEmail, assertBookingId, extractLookupUsers, normalizeUser, emailsForUser,
  toDate, getCheckInWindowState, createToken, hashToken, CHECK_IN_WINDOW_MS,
};
