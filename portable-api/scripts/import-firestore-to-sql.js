...
const date = (value) => {
  ...
  const rawSeconds = value?._seconds ?? value?.seconds;
  const seconds = Number(rawSeconds);
  if (value && rawSeconds !== undefined && Number.isFinite(seconds)) {
    const nanos = Number(value._nanoseconds ?? value.nanoseconds);
    const milliseconds = (seconds * 1000) + Math.floor((Number(nanos) || 0) / 1_000_000);
...
const requiredText = (value, max, name) => {
  const result = text(value, max).trim();
  if (!result) throw new Error(`Missing or invalid ${name}.`);
  return result;
};
const requiredDate = (value, name) => {
  const result = date(value);
  if (!result || Number.isNaN(new Date(result).getTime())) throw new Error(`Missing or invalid ${name}.`);
  return result;
};
const dateOnly = (value, name) => {
  const result = requiredText(value, 10, name);
  const parsed = new Date(`${result}T00:00:00.000Z`);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(result) || Number.isNaN(parsed.getTime()) || parsed.toISOString().slice(0, 10) !== result) throw new Error(`Missing or invalid ${name}.`);
  return result;
};
const normalizeForBackup = (value) => {
  const timestamp = date(value);
  if (timestamp !== value) return timestamp;
  if (Array.isArray(value)) return value.map(normalizeForBackup);
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, normalizeForBackup(entry)]));
  return value;
};
const status = (value) => {
...
async function importBooking(id, value, transaction) {
  ...
  const queueDetails = legacyBookingEmailQueueDetails({
    status: value.status,
    startTime: start,
    actualStartTime: date(value.actualStartTime),
    verificationEmailStatus: value.verificationEmailStatus,
    verificationEmailScheduledAt: date(value.verificationEmailScheduledAt),
  });
  if (!queueDetails) return;
  await new sql.Request(transaction)
    .input('bookingId', sql.NVarChar(128), id)
    .input('scheduledAt', sql.DateTime2, queueDetails.scheduledAt)
    .input('opensAt', sql.DateTime2, queueDetails.opensAt)
    .input('closesAt', sql.DateTime2, queueDetails.closesAt)
    .query(`UPDATE dbo.Bookings
      SET VerificationEmailScheduledAt = COALESCE(VerificationEmailScheduledAt, @scheduledAt),
          VerificationWindowOpenedAt = COALESCE(VerificationWindowOpenedAt, @opensAt),
          VerificationWindowClosedAt = COALESCE(VerificationWindowClosedAt, @closesAt),
          UpdatedAt = SYSUTCDATETIME()
      WHERE Id = @bookingId;
      INSERT INTO dbo.EmailQueue (BookingId, ScheduledAt, Status)
      SELECT @bookingId, @scheduledAt, N'queued'
      WHERE NOT EXISTS (SELECT 1 FROM dbo.EmailQueue WHERE BookingId = @bookingId);`);
}

async function importMaintenanceHistory(id, value, transaction) {
  const roomId = requiredText(value.roomId, 128, 'maintenance room ID');
  const startDate = dateOnly(value.startDate, 'maintenance start date');
  const endDate = dateOnly(value.endDate, 'maintenance end date');
  const startTime = integer(value.startTime); const endTime = integer(value.endTime);
  if (endDate < startDate || startTime === null || endTime === null || startTime < 0 || endTime > 24 || endTime <= startTime) throw new Error('Missing or invalid maintenance range.');
  await new sql.Request(transaction)
    .input('id', sql.NVarChar(180), id).input('roomId', sql.NVarChar(128), roomId).input('roomName', sql.NVarChar(200), requiredText(value.roomName, 200, 'maintenance room name'))
    .input('reason', sql.NVarChar(200), requiredText(value.reason, 200, 'maintenance reason')).input('startDate', sql.Date, startDate).input('endDate', sql.Date, endDate)
    .input('startTime', sql.Int, startTime).input('endTime', sql.Int, endTime).input('createdAt', sql.DateTime2, date(value.createdAt))
    .query(`MERGE dbo.RoomMaintenanceHistory WITH (HOLDLOCK) AS target USING (SELECT @id AS Id) AS source ON target.Id=source.Id
      WHEN MATCHED THEN UPDATE SET RoomId=@roomId,RoomName=@roomName,Reason=@reason,StartDate=@startDate,EndDate=@endDate,StartTime=@startTime,EndTime=@endTime,CreatedAt=COALESCE(@createdAt,target.CreatedAt)
      WHEN NOT MATCHED THEN INSERT (Id,RoomId,RoomName,Reason,StartDate,EndDate,StartTime,EndTime,CreatedAt) VALUES (@id,@roomId,@roomName,@reason,@startDate,@endDate,@startTime,@endTime,COALESCE(@createdAt,SYSUTCDATETIME()));`);
}

async function importMissedCheckInHistory(id, value, transaction) {
  const archivedAt = requiredDate(value.archivedAt || value.missedCheckInAt || value.createdAt, 'missed check-in archive date');
  const originalBookingId = requiredText(value.originalBookingId || value.id || id, 128, 'original booking ID');
  await new sql.Request(transaction)
    .input('id', sql.NVarChar(128), id).input('bookingId', sql.NVarChar(128), originalBookingId)
    .input('payload', sql.NVarChar(sql.MAX), JSON.stringify(normalizeForBackup(value))).input('archivedAt', sql.DateTime2, archivedAt)
    .input('reason', sql.NVarChar(200), text(value.archivedReason, 200) || 'Missed check-in window')
    .query(`MERGE dbo.MissedCheckInHistory WITH (HOLDLOCK) AS target USING (SELECT @id AS Id) AS source ON target.Id=source.Id
      WHEN MATCHED THEN UPDATE SET OriginalBookingId=@bookingId,Payload=@payload,ArchivedAt=@archivedAt,ArchiveReason=@reason
      WHEN NOT MATCHED THEN INSERT (Id,OriginalBookingId,Payload,ArchivedAt,ArchiveReason) VALUES (@id,@bookingId,@payload,@archivedAt,@reason);`);
}

async function importEmailSentHistory(id, value, transaction) {
  const sentAt = date(value.sentAt || value.createdAt);
  const createdAt = date(value.createdAt || value.sentAt);
  await new sql.Request(transaction)
    .input('id', sql.UniqueIdentifier, crypto.randomUUID()).input('documentId', sql.NVarChar(180), id)
    .input('email', sql.NVarChar(254), text(value.recipientEmail, 254) || 'unknown@yageo.com').input('name', sql.NVarChar(200), text(value.recipientName, 200))
    .input('subject', sql.NVarChar(500), text(value.subject, 500) || 'SmartRoom email').input('purpose', sql.NVarChar(100), text(value.purpose, 100) || 'Email')
    .input('status', sql.NVarChar(20), text(value.status, 20) || 'unknown').input('bookingId', sql.NVarChar(128), text(value.relatedBookingId, 128) || null)
    .input('bookingTitle', sql.NVarChar(500), text(value.relatedBookingTitle, 500) || null).input('roomId', sql.NVarChar(128), text(value.relatedRoomId, 128) || null)
    .input('roomName', sql.NVarChar(500), text(value.relatedRoomName, 500) || null).input('errorCode', sql.NVarChar(100), text(value.errorCode, 100) || null)
    .input('errorMessage', sql.NVarChar(1000), text(value.errorMessage, 1000) || null).input('sentAt', sql.DateTime2, sentAt).input('createdAt', sql.DateTime2, createdAt)
    .query(`MERGE dbo.EmailAudit WITH (HOLDLOCK) AS target USING (SELECT @documentId AS FirestoreDocumentId) AS source ON target.FirestoreDocumentId=source.FirestoreDocumentId
      WHEN MATCHED THEN UPDATE SET RecipientEmail=@email,RecipientName=@name,Subject=@subject,Purpose=@purpose,Status=@status,RelatedBookingId=@bookingId,RelatedBookingTitle=@bookingTitle,RelatedRoomId=@roomId,RelatedRoomName=@roomName,ErrorCode=@errorCode,ErrorMessage=@errorMessage,SentAt=@sentAt,CreatedAt=COALESCE(@createdAt,target.CreatedAt)
      WHEN NOT MATCHED THEN INSERT (Id,FirestoreDocumentId,RecipientEmail,RecipientName,Subject,Purpose,Status,RelatedBookingId,RelatedBookingTitle,RelatedRoomId,RelatedRoomName,ErrorCode,ErrorMessage,SentAt,CreatedAt) VALUES (@id,@documentId,@email,@name,@subject,@purpose,@status,@bookingId,@bookingTitle,@roomId,@roomName,@errorCode,@errorMessage,@sentAt,COALESCE(@createdAt,SYSUTCDATETIME()));`);
}

async function importAnnouncement(id, value, transaction) {
  const startAt = requiredDate(value.startAt, 'announcement start date'); const endAt = requiredDate(value.endAt, 'announcement end date');
  if (new Date(endAt) <= new Date(startAt)) throw new Error('Invalid announcement date range.');
  const targetPages = [...new Set(Array.isArray(value.targetPages) ? value.targetPages.map((page) => text(page, 50).trim()).filter(Boolean) : ['all'])];
  await new sql.Request(transaction)
    .input('id', sql.NVarChar(128), id).input('title', sql.NVarChar(200), requiredText(value.title, 200, 'announcement title')).input('message', sql.NVarChar(sql.MAX), requiredText(value.message, 2000, 'announcement message'))
    .input('category', sql.NVarChar(40), text(value.category, 40) || 'info').input('imageUrl', sql.NVarChar(sql.MAX), text(value.imageUrl, 2048)).input('buttonText', sql.NVarChar(100), text(value.buttonText, 100))
    .input('buttonUrl', sql.NVarChar(2048), text(value.buttonUrl, 2048)).input('startAt', sql.DateTime2, startAt).input('endAt', sql.DateTime2, endAt).input('isActive', sql.Bit, Boolean(value.isActive))
    .input('showOnce', sql.Bit, Boolean(value.showOnce)).input('targetPages', sql.NVarChar(sql.MAX), JSON.stringify(targetPages)).input('audience', sql.NVarChar(30), text(value.audience, 30) || 'all')
    .input('priority', sql.Int, integer(value.priority, 0)).input('createdAt', sql.DateTime2, date(value.createdAt)).input('updatedAt', sql.DateTime2, date(value.updatedAt)).input('publishedAt', sql.DateTime2, date(value.publishedAt)).input('disabledAt', sql.DateTime2, date(value.disabledAt))
    .query(`MERGE dbo.Announcements WITH (HOLDLOCK) AS target USING (SELECT @id AS Id) AS source ON target.Id=source.Id
      WHEN MATCHED THEN UPDATE SET Title=@title,Message=@message,Category=@category,ImageUrl=@imageUrl,ButtonText=@buttonText,ButtonUrl=@buttonUrl,StartAt=@startAt,EndAt=@endAt,IsActive=@isActive,ShowOnce=@showOnce,TargetPages=@targetPages,Audience=@audience,Priority=@priority,UpdatedAt=COALESCE(@updatedAt,target.UpdatedAt),PublishedAt=@publishedAt,DisabledAt=@disabledAt
      WHEN NOT MATCHED THEN INSERT (Id,Title,Message,Category,ImageUrl,ButtonText,ButtonUrl,StartAt,EndAt,IsActive,ShowOnce,TargetPages,Audience,Priority,CreatedAt,UpdatedAt,PublishedAt,DisabledAt) VALUES (@id,@title,@message,@category,@imageUrl,@buttonText,@buttonUrl,@startAt,@endAt,@isActive,@showOnce,@targetPages,@audience,@priority,COALESCE(@createdAt,SYSUTCDATETIME()),COALESCE(@updatedAt,SYSUTCDATETIME()),@publishedAt,@disabledAt);`);
}

async function main() {
  const collections = ['rooms', 'bookings', 'roomMaintenanceHistory', 'missedCheckInHistory', 'emailSentHistory', 'announcements'];
  let backup;
  if (backupInputPath) {
    backup = JSON.parse(fs.readFileSync(backupInputPath, 'utf8'));
    if (!backup?.collections || ['rooms', 'bookings'].some((name) => !Array.isArray(backup.collections[name]))) {
      throw new Error('Backup has an invalid SmartRoom migration format.');
    }
    for (const name of collections) backup.collections[name] ||= [];
    ...
    for (const name of collections) {
      ...
      backup.collections[name] = snapshot.docs.map((doc) => ({ id: doc.id, data: normalizeForBackup(doc.data()) }));
    }
  ...
  try {
    ...
    const imports = [
      ['roomMaintenanceHistory', importMaintenanceHistory],
      ['missedCheckInHistory', importMissedCheckInHistory],
      ['emailSentHistory', importEmailSentHistory],
      ['announcements', importAnnouncement],
    ];
    for (const [collection, importRecord] of imports) {
      for (const record of backup.collections[collection]) {
        await importRecord(record.id, record.data, transaction);
        await audit(transaction, collection, record.id, 'imported');
      }
    }
